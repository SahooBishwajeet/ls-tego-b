...
def run():
    ...
    for out_format in out_formats:
        assert test_convert(message, out_format), (
            "Format: " + out_format + " was unsuccessful!"
        )
...
def test_convert(message, out_format):
    ...
    logger.info(
        "Audio info: Filename: %s FrameCount: %s FrameWidth: %s SampleWidth: %s",
        infile,
        wav_audio.frame_count(),
        wav_audio.frame_width,
    ...
    logger.info(
        ...
        steg_audio.frame_width,
        steg_audio.sample_width,
    )
