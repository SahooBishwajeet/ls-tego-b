...
def convert_to_stego_image(image, message):
    ...
    updated_bytes = convert_to_stego_bitarray(
        _image_to_bitarray(old_pixel_data), message_bits, _get_bits_per_pixel(image)
    ).tobytes()
...
def convert_to_stego_audio(audio, message):
    ...
    new_audio_data = convert_to_stego_bitarray(
        _audio_to_bitarray(old_audio_data), message_bits, bits_per_sample
    )
...
def convert_to_stego_bitarray(container_bitarray, message_bits, bits_per_sample):
    ...
    end = len(message_bits) * bits_per_sample
...
def convert_from_stego_image(image):
    ...
    return convert_from_stego_bitarray(
        _image_to_bitarray(image.getdata()), _get_bits_per_pixel(image)
    )
...
def convert_from_stego_audio(audio):
    ...
    bits_per_sample = audio.sample_width * 8
    return convert_from_stego_bitarray(
        _audio_to_bitarray(audio.raw_data), bits_per_sample
    )
